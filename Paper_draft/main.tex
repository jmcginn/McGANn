...
LIGO and Advanced Virgo \cite{Prospects-dets, AdvLIGO, AdvLIGO2, AdvVIRGO} ~\chris{need refs for the detectors, see here
https://pnp.ligo.org/ppcomm/standard\_references.html} ~\jordan{\ding{51}} reported several more
...
post-mergers. Currently, \ac{GW}s are detected using matched filtering, \cite{Owen1998} a process which requires template banks of possible GW signals. As \ac{GW} bursts are un-modelled there are no templates available and so these signals are undetectable through this scheme. ~\chris{OK, but logically dodgy. You make it
sound like we could use a template scheme but it would be insensitive. The
point is that we don't have templates in the first place.} \jordan{changed this but still reads clunky} Instead, detection involves distinguishing
...
an astrophysical burst signature appearing in multiple detectors at similar times. ~\chris{at
similar times?}. \ding{51}  

% Discuss the family of burst waveforms currently used and why - not in detail, just
% an introduction 
%
\ac{GW} burst algorithms \cite{Klimenko_2008, Aso_2008}~\chris{many? needs
...
and share characteristics of real bursts that is enough to simulate a \ac{GW} passing ~\chris{fancy sounding but I'm not sure what
non-stationary deviations means.} between detectors. 
...
With the expectation that there will be many more \ac{GW} detections in the future, researches are starting to realise the need for fast and efficient \ac{GW} analysis to compete with the rising number of detections. Machine learning has shown great potential for \ac{GW} analysis in areas of detection \cite{george2016deep,Gabbard2017,Gebhard_2019,Krastev_2020}, glitch classification \cite{Bahaadini, George_2018,Razzano_2018} and parameter estimation \cite{gabbard2019bayesian,shen2019deterministic,green2020gravitationalwave}. Applying these machine learning algorithms show similar results to matched filtering techniques and have the benefit of being orders of magnitude faster. 
...
unmodelled \ac{GW} burst waveforms. \jordan{i haven't mentioned gans yet do i explain it here briefly?}
...
weights, they haven't been defined} \jordan{defined above but went on a bit, should i go into back prop and optimizers?} of the discriminator and generator based on
